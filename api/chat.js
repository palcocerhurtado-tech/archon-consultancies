...
function buildContextMessages(body) {
  ...
  if (savedDiagnosis && savedDiagnosis.summaryText) {
    contextMessages.push({
      role: "system",
      content: "Ultimo diagnostico guardado del usuario:\n" + normalizeString(savedDiagnosis.summaryText)
    });
  }
...
function extractAssistantText(payload) {
  ...
  return payload.choices
    .map(function (choice) {
      return choice && choice.message ? normalizeString(choice.message.content) : "";
    })
    .filter(Boolean)
    .join("\n");
}
...
export async function POST(request) {
  ...
  const messages = [{ role: "system", content: SYSTEM_PROMPT }]
    .concat(buildContextMessages(body))
    .concat(history)
    .concat([{ role: "user", content: message }]);

  let upstreamResponse;
  try {
    upstreamResponse = await fetch(NVIDIA_API_URL, {
      method: "POST",
      headers: {
    ...
    return jsonResponse({ error: "Could not reach NVIDIA API." }, 502);
  }
