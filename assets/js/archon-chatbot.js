(function () {
  ...
  async function checkApiAvailability() {
    try {
      ...
      if (state.aiAvailable) {
        setStatus("IA NVIDIA conectada. Puedes conversar o pedir tu borrador.", "live");
      } else {
        setStatus("Modo local activo. La IA se activara al configurar NVIDIA_API_KEY.", "fallback");
      }
    } catch (error) {
  ...
  async function askNvidia(payload) {
    const session = loadSession();
  ...
  function renderApiResponse(data) {
    ...
    if (data.reportReady && data.report) {
      ...
      hydrateForms();
      renderReportCard(report, "NVIDIA Llama 3.1 70B");
    }
    ...
    setStatus("IA NVIDIA conectada. Sigo contigo.", "live");
    return true;
  }
  ...
  async function finishDiagnosis() {
    ...
    removePendingOptions();
    setStatus("Construyendo borrador con NVIDIA Llama 3.1 70B...", "thinking");

    const apiResponse = await askNvidia({
    ...
    if (apiResponse && apiResponse.reportReady && apiResponse.report) {
      ...
      renderReportCard(finalReport, "NVIDIA Llama 3.1 70B");
      addOptions([
        { label: "Reservar revision", action: "audit" },
        { label: "Copiar resumen", action: "copy" },
        { label: "Nuevo analisis", action: "startDiagnosis" }
  ...
  async function handleFreeText(text) {
    ...
    setStatus("Modo local activo. La IA se activara cuando NVIDIA_API_KEY este configurada.", "fallback");
  }
